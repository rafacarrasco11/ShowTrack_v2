...
public class APINews {
    ...
    public static List<New> getNewsOfTheDay()  {
        ...
        newsList.addAll(getNewsSeries());
        newsList.addAll(getNewsPeliculas());
    ...
    private static List<New> getNewsSeries() {
        List<New> newsSeriesList = new ArrayList<>();

        JSONNews jsonNews = null;
        try {
            jsonNews = (JSONNews) APIUtil.getInstance().getFromApi(JSONNews.class, generateURL(ShowTrackApplication.context().getString(R.string.keyword_series_apinews)));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static List<New> getNewsPeliculas() {
        List<New> newsPeliculasList = new ArrayList<>();

        JSONNews jsonNews = null;
        try {
            jsonNews = (JSONNews) APIUtil.getInstance().getFromApi(JSONNews.class, generateURL(ShowTrackApplication.context().getString(R.string.keyword_peliculas_apinews)));
        } catch (IOException e) {
            e.printStackTrace();
        }
